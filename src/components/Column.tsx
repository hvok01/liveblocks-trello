...
export default function Column({id, name}: ColumnProps) {
    ...
    return (
        <div key={id} className="w-48 bg-white shadow-sm rounded-md p-2">
            {
                !renameMode && (
            ...
            }
            {
                renameMode && (
                    <div className="mb-8">
                        ...
                        <button className="bg-red-500 text-white p-2 flex gap-2 w-full items-center rounded-md justify-center" onClick={() => deleteColumn(id)}>
            ...
            }
            {
                !renameMode && columnCards && (
                    <>
                    <ReactSortable list={columnCards} setList={items => setTasksOrderForColumn(items, id)} group="cards" className="min-h-12" ghostClass="opacity-40">
                        {
                            columnCards.map((card, id) => (
                                <ColumnCard id={card.id} name={card.name} key={id}/>
                            ))
                        }
                    </ReactSortable>
                    </>
